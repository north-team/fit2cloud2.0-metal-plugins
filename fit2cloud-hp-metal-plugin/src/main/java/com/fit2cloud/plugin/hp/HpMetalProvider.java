...
import com.fit2cloud.plugin.hp.model.*;
import com.fit2cloud.plugin.hp.utils.HpHttpFutureUtils;
...
public class HpMetalProvider extends AbstractMetalProvider {
    ...
    public HpMetalProvider() {
        super.name = "fit2cloud-hp-metal-plugin";
    }
                ...
                if (phyNicDTO != null && phyNicDTO.getPhyNics().size() > 0) {
                    for (HpPhyNicDTO.PhyNicsBean phyNic : phyNicDTO.getPhyNics()) {
                        ...
                        phyNic.getPorts().forEach(p -> {
                            F2CPmNetworkCard card = new F2CPmNetworkCard();
    ...
    public JSONObject getRaidPayLoad(String raidConfigRequestStr) throws MetalPluginException {
        ...
        for (int i = 0; i < raidConfigList.size(); i++) {
            ...
            StringBuffer drives2 = new StringBuffer();
            raidConfig.getRaidDisks().stream().sorted(Comparator.comparing(F2CPmDisk::getDrive)).forEach(physicalDisk -> {
                drives2.append(physicalDisk.getDrive()).append(",");
            });
            raid.put("drives", drives2.substring(0, drives2.length() - 1));
            raidList.add(raid);
        }
    ...
    public F2CPmMetric getMetric(String ipmiSnmpRequestStr) throws MetalPluginException {
        ...
        F2CPmMetric f2CBareMetalMetric = new F2CPmMetric();
        try {

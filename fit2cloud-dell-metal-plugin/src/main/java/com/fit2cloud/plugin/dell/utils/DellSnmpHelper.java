...
public class DellSnmpHelper {
    ...
    private List<F2CPmCpu> getCpus() throws MetalPluginException {
        ...
        for (Map.Entry<String, String> entry : nameMap.entrySet()) {
            if (entry.getKey() != null) {
                F2CPmCpu cpuEntity = new F2CPmCpu();
    ...
    public MachineEntity getMachineEntry() throws MetalPluginException {
        try {
            ...
            // 获取cpu相关
            List<F2CPmCpu> cpus = getCpus();
            entry.setPmCpus(cpus);
            List<F2CPmMemory> memories = getMemorys();
            entry.setPmMemories(memories);
            List<F2CPmDisk> disks = getDisks();
            entry.setDisks(disks);
            List<F2CPmNetworkCard> networkCards = getNetworkCards();
    ...
    public List<F2CPmDisk> getDisks() {
        List<F2CPmDisk> disks = new LinkedList<>();
        try {
    ...
    private List<F2CPmNetworkCard> getNetworkCards() {
        List<F2CPmNetworkCard> cards = new LinkedList<>();
        try {
            ...
            for (Map.Entry<String, String> entry : cardMap.entrySet()) {
                if (entry.getKey() != null) {
                    F2CPmNetworkCard d = new F2CPmNetworkCard();
                    d.setSynTime(now);
                    d.setNumber("eth" + i);
    ...
    private List<F2CPmMemory> getMemorys() {
        ...
        try {
            ...
            for (Map.Entry<String, String> entry : sizeMap.entrySet()) {
                if (entry.getKey() != null) {
                    F2CPmMemory pmMemory = new F2CPmMemory();

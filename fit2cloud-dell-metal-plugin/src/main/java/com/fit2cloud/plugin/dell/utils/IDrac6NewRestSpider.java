...
import com.fit2cloud.plugin.dell.model.DellCpuDTO;
import com.fit2cloud.plugin.dell.model.DellMemoryDTO;
...
public class IDrac6NewRestSpider implements IIDRACRestAPI {

    public static final String overviewUrl = "https://%s/data?get=sysDesc,biosVer,svcTag,expSvcCode,hostName,osName,osVersion,sysRev,LCCfwVersion,recoveryAction,initCountdown,presentCountdown,datetime,fwVersion,fwUpdated,hwVersion,macAddr,v4Enabled,v4IPAddr,v4Gateway,v4NetMask,v4DHCPEnabled,v4DHCPServers,v4DNS1,v4DNS2,v6Enabled,v6Addr,v6Gateway,v6DHCPEnabled,v6LinkLocal,v6Prefix,v6SiteLocal,v6SiteLocal3,v6SiteLocal4,v6SiteLocal5,v6SiteLocal6,v6SiteLocal7,v6SiteLocal8,v6SiteLocal9,v6SiteLocal10,v6SiteLocal11,v6SiteLocal12,v6SiteLocal13,v6SiteLocal14,v6SiteLocal15,racName,v6DHCPServers,v6DNS1,v6DNS2,NicEtherMac1,NicEtherMac2,NicEtherMac3,NicEtherMac4,NiciSCSIMac1,NiciSCSIMac2,NiciSCSIMac3,NiciSCSIMac4,NicEtherVMac1,NicEtherVMac2,NicEtherVMac3,NicEtherVMac4,";
    ...
    public boolean loginCurl(String ip, String userName, String password) {
        try {
            ...
            Result result = DellUtils.login(ip, handleGermanChar(userName), handleGermanChar(password));
...
//
//            String entityString = String.format("user=%s&password=%s", handleGermanChar(userName), handleGermanChar(password));
...
//                LogUtil.info(String.format("爬取Dell idrac6 %s登录接口返回结果forwardUrl：%s", ip, forwardUrl));
//                headers.put("ST2", getST2(forwardUrl));
//                RequestUtils.idrac6FormatHeaders(ip, headers);
//                headersMap.put(ip, headers);
//                return true;
//            } else {
//                LogUtil.error(String.format("爬取Dell idrac6 %s硬件信息时登录失败！可能是账号密码不对！%s", ip, entityString));
//                return false;
//            }
//        } catch (Exception e) {
//            LogUtil.error(String.format("爬取Dell idrac6 %s硬件信息时，获取重定向地址失败！可能是达到了最大爬取次数！清空缓存！e:%s", ip, ExceptionUtils.getExceptionDetail(e)));
